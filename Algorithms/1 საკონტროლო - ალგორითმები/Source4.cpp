...
public:
	...
	bool DyeInTwoColors()
	{
		...
		{
			if (!visited[i]) // თუ წვერო მოუნახულებელია
			{
				COLOR[i] = 1; // მაშინ წვეროს, საიდანაც BFS დაიწყება ვაფერადებთ თეთრად
				BFS(IntoChar(i)); // და ვუშვებთ ალგორითმს. ასე მოვივლით ყველა წვეროს გრაფში და განვუსაზღვრავთ ფერებს
			}
		}
		
		for (int i = 0; i < V; ++i) // გრაფის ყოველი წვეროსთვის
		{
			for (auto it = adj[i].begin(); it != adj[i].end(); ++it) // წვეროს ყოველი მეზობელი წვეროსთვის
			{
				if (COLOR[i] == COLOR[*it]) // თუ ეს წვერო და მისი მეზობელი წვერო ერთი ფერისაა, მაშინ გრაფი ვერ შეღებილა
					return false;
			}
		}

		return true; // თუ კოდი აქამდე მოვიდა, ესეიგი გრაფი წარმატებით შეიღება
	}
	...
	void BFS(char s)
	{
		...
		while (!queue.empty())
		{
			...
			cout << '\'' << IntoChar(S) << "\'  <->  " << d[S] << "  <->  \'" << parent[S] << "\' <-> " << GetColor(COLOR[S]) << endl; // დავბეჭდოთ
